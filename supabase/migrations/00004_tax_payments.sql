-- ── Tax Payments ──────────────────────────────────────────────────────────────
-- Records actual installment and balance-owing payments to CRA and Revenu Québec
...
CREATE TABLE IF NOT EXISTS tax_payments (
  ...
  due_amount       DECIMAL(12,2),               -- expected amount for this quarter
...
CREATE INDEX IF NOT EXISTS idx_tax_payments_org_year
  ON tax_payments (organization_id, tax_year);

ALTER TABLE tax_payments ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can manage their org tax payments"
  ON tax_payments
  USING (
    organization_id IN (
